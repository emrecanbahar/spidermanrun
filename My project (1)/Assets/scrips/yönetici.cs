...
public class yönetici : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        uretme(altin, 30, altinlar);

        uretme(miknatis, 30, digerleri);
        uretme(kutuk, 30, digerleri);
        uretme(tas, 30, digerleri);
        uretme(araba, 25, digerleri);
    ...
    void engel_uret()
    {
        ...
        if (digerleri[rast].activeSelf == false)
        {
            ...
            if (rastgele == 0)
            {
                digerleri[rast].transform.position = new Vector3(7.0f, -3.0f, cocuk.position.z + 30.0f);
            }

            if (rastgele == 1)
            {
                digerleri[rast].transform.position = new Vector3(-3.30f, -3.0f, cocuk.position.z + 30.0f);
            }
            ...
            if (digerleri[rast].tag == "miknatis")
            {
                ...
                if (cocuk.gameObject.GetComponent<karakter_kontorl>().miknatis_alindi == true)
                {
        ...
        else
        {
            foreach (GameObject nesne in digerleri)
            {
                if (nesne.activeSelf == false)
                {
                    ...
                    if (rastgele_2 == 0)
                    {
                        nesne.transform.position = new Vector3(-3.30f, 14.0f, cocuk.position.z + 30.0f);
                    }

                    if (rastgele_2 == 1)
                    {
                        nesne.transform.position = new Vector3(7.0f, 14.0f, cocuk.position.z + 30.0f);
                    }
    ...
    void altin_uret()
    {
        foreach (GameObject altin in altinlar)
        {
            if (altin.activeSelf == false)
            {
                ...
                if (rastgele == 0)
                {
                    altin.transform.position = new Vector3(-3.30f, -3.0f, cocuk.position.z + 10.0f);
                }

                if (rastgele == 1)
                {
                    altin.transform.position = new Vector3(7.0f, -3.0f, cocuk.position.z + 10.0f);
                }
